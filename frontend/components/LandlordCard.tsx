...
export default function LandlordCard({ landlord }: LandlordCardProps) {
  ...
  return (
    <Link href={`/landlord/${landlord.slug}`} className="block group">
      <Card className="transition-colors duration-200 border-white/50 group-hover:border-accent bg-white/60 backdrop-blur-xl rounded-2xl shadow-card">
        <CardContent className="p-6 md:p-8 flex flex-col md:flex-row items-start justify-between gap-6 pb-6 pt-6 md:pb-8 md:pt-8 w-full border-0">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h3 className="text-2xl font-bold tracking-tight text-text-heading">
                {landlord.name}
              </h3>
              {landlord.review_count > 0 && (
                <span className="px-2 py-0.5 text-xs font-bold uppercase tracking-wider bg-bg-subtle text-text-body border border-blue-100 rounded-md">
                  {landlord.review_count} {landlord.review_count === 1 ? "Review" : "Reviews"}
                </span>
              )}
            </div>

            <div className="text-sm font-medium flex items-center gap-2 mb-4 text-text-muted">
              <span className="text-base text-accent">📍</span> {landlord.city}
          ...
          <div className="flex-shrink-0 text-center px-6 py-4 rounded-2xl bg-white/50 border border-white/60 shadow-sm">
            <div className="text-xs font-bold uppercase tracking-widest mb-1 text-accent">Rating</div>
